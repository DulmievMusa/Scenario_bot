...
def choose_language(message, user):
    ...
    try:
        ...
        if user.ui_language == 'ru':
            langs = sorted(config.dict_of_short_languages_ru.keys())
        else:
            langs = sorted(config.dict_of_short_languages_eng.keys())
    ...
    try:
        if message.text.lower() not in user.sp_of_langs and message.text not in ['/chanlan',
            ...
            bot.send_message(message.chat.id, 'Select interface language\nВыберите язык интерфейса',
                             reply_markup=user.markup_language)
            bot.register_next_step_handler(message, choose_language, user)
            return
...
def main_process(message, user, markup):
    ...
    try:
        ...
        if user.mix:
            shuffle(final_text)
        test_text = '; '.join(final_text)
        test_text = translator.translate(test_text, src='en', dest=user.language_to_translate).text
        translated_text = test_text.split('; ')
        final = ''
        if len(final_text) > len(translated_text):
            final_text = final_text[:-(len(final_text) - len(translated_text))]
        for i in range(len(final_text)):
            final += f'{final_text[i]} - {translated_text[i]}\n'
        ...
        if len(final) > 4095:
            for x in range(0, len(final), 4095):
                bot.reply_to(message, text=final[x:x + 4095])
        else:
            bot.reply_to(message, text=final)
        # markup = types.ReplyKeyboardMarkup(selective=True)
        # btn = types.KeyboardButton(user.slovar['start_again_button'])
        # markup.add(btn)
        # bot.send_message(message.chat.id, user.slovar['send_scenario'], reply_markup=markup)
