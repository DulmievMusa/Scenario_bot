	...
	try:
		if message.text == 'Русский':
			slovar = {'ok': 'Хорошо',
					  ...
					  'help': 'Этот бот пригодится когда вам нужно заранее перевести сложные слова из текста на английском. Он удалить из текста все простые слова и отправит только сложные.'
...
def mix_question(message, user):
	chat_id = message.chat.id
	try:
		if message.text not in ['Да', 'Yes', 'No', 'Нет'] and message.text not in ['/chanlan', '/help']:
			raise WrongWord
		elif message.text == '/chanlan':
			markup = types.ReplyKeyboardMarkup(selective=True)
			btn1 = types.KeyboardButton('Русский')
